...
pub(crate) fn euclidean_distance<T>(first: &[T], second: &[T]) -> Option<T>
where
...
pub(crate) fn two_asc_rnd_num<R, T>(rng: &mut R, range: Range<T>) -> [T; 2]
where
  R: Rng,
...
pub(crate) fn two_dist_rnd_num<R, T>(rng: &mut R, range: Range<T>) -> [T; 2]
where
  R: Rng,
  T: Copy + SampleUniform + PartialOrd,
{
